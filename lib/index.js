...
import { makeConfigRoutes, migrateLegacyPaths, readShellConfig, storedNotifyOnTaskComplete } from './services/config-api.js';
...
export function apply(ctx, config) {
    ...
    ctx.on('session/event', (session, event) => {
        ...
        if ((session.header?.delegationDepth ?? 0) !== 0)
            return;
        const e = event;
        if (e?.type !== 'turn/end' || e.data?.reason?.kind !== 'completed')
            return;
        try {
