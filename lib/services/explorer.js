 ...
 */
function loadShellOpenApi() {
    try {
        // eslint-disable-next-line @typescript-eslint/no-require-imports
        const koffi = require('koffi');
        const shell32 = koffi.load('shell32.dll');
        const shellExecuteW = shell32.func('intptr_t ShellExecuteW(void* hwnd, str16 operation, str16 file, str16 parameters, str16 directory, int show)');
        return (path) => {
            // SW_SHOWNORMAL = 1
...
function loadFocusApi() {
    try {
        ...
        const getWindowTextLengthW = user32.func('int GetWindowTextLengthW(HWND hwnd)');
        const getWindowTextW = user32.func('int GetWindowTextW(HWND hwnd, _Out_ char16_t *buf, int max)');
        const getClassNameW = user32.func('int GetClassNameW(HWND hwnd, _Out_ char16_t *buf, int max)');
        const setForegroundWindow = user32.func('bool SetForegroundWindow(HWND hwnd)');
        const bringWindowToTop = user32.func('bool BringWindowToTop(HWND hwnd)');
        const tryFocus = (title) => {
            let found = null;
            enumWindows((hwnd) => {
                if (found !== null)
                    return false;
...
function runPowerShellFocus(folderPath) {
    try {
...
function focusExplorerWindow(folderPath) {
