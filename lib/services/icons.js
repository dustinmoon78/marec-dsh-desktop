 ...
 */
export function loadPngRgba(posixPath) {
    try {
        const { data, width, height } = decodePngRgba(fileURLToPath(new URL(posixPath, import.meta.url)));
...
export function dshFaviconDark() {
 ...
 */
export function downscaleIcon(icon, size) {
 ...
 */
export function dshFaviconTray(dark) {
