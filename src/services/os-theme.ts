...
let cachedLight: boolean | undefined

/** Parse `reg query` output like `SystemUsesLightTheme REG_DWORD 0x1`. */
function parseDword(stdout: string): boolean | undefined {
  const m = /0x([0-9a-f]+)/i.exec(stdout)
  if (m === null) return undefined
  return (parseInt(m[1], 16) & 1) === 1
}

function queryValue(valueName: string): boolean | undefined {
  try {
 ...
 */
export function osThemeIsLight(): boolean | undefined {
  if (cachedLight !== undefined) return cachedLight
...
export function refreshOsTheme(): void {
