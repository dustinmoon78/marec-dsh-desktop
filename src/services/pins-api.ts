 ...
 *
 * Persistence file: $DSH_HOME/mg-dsh-desktop/pins.json → { "ids": string[] }
 * (ordered, deduped, capped).
 */
...
/** Route prefix shared with the other mg-dsh-desktop APIs. */
const API_PREFIX = '/api/mg-dsh-desktop'
...
export function pinsFile(): string {
  return join(dshHome(), 'mg-dsh-desktop', 'pins.json')
}
...
/** Persist the pinned session ids (best-effort, atomic write). */
