...
interface ClientCtxLike {
  ...
  workspaces?: {
    list?: {
      subscribe?: (cb: () => void) => () => void
      getSnapshot?: () => WorkspacesListLike | undefined
    }
  }
}
...
export function installPinnedConversations(ctx: unknown): () => void {
  ...
  const dirtyDelta = { added: new Set<string>(), removed: new Set<string>() }
  ...
  function workspaceSnapshot(): WorkspacesListLike | undefined {
  ...
  function mapRowByContent(row: HTMLElement): { id: string; titleEl: Element } | undefined {
    ...
    for (const summary of Object.values(byId)) {
      ...
      if (list === undefined) titleToIds.set(key, [summary.id ?? ''])
  ...
  function ensurePinButton(row: HTMLElement, id: string, titleEl: Element): HTMLButtonElement {
    // Compare via dataset, never interpolate the id into a selector.
    let button = Array.from(row.querySelectorAll<HTMLButtonElement>('[data-mg-pin]'))
  ...
  function applyRowState(row: HTMLElement, id: string, titleEl: Element): void {
  ...
  function pruneStaleButtons(row: HTMLElement, id: string | undefined): void {
  ...
  return () => {
    ...
    for (const el of Array.from(document.querySelectorAll<HTMLElement>(`[data-mg-pinned]`))) {
